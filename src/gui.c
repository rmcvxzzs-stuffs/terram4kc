 ...
 * string.
 */
void strnum(char *ptr, int offset, int num) { sprintf(ptr + offset, "%d", num); }
 ...
 */
int button(SDL_Renderer *renderer, const char *str, int x, int y, int w, int mouseX, int mouseY) {
	int hover = mouseX >= x && mouseY >= y && mouseX < x + w && mouseY < y + 16;
 ...
 */
int input(SDL_Renderer *renderer, const char *placeholder, const char *buffer, int x, int y, int w,
          int mouseX, int mouseY, int active) {
	static int flash = 0;
 ...
 */
void scrollbar(SDL_Renderer *renderer, int x, int y, int length, int mouseX, int mouseY,
               int mouseLeft, int *level, int max) {
	float sectionLength = (float)length / (float)max;

	SDL_Rect background = {.x = x, .y = y, .w = 4, .h = length};

	SDL_Rect foreground = {
	    .x = x, .y = (float)*level * sectionLength, .w = 4, .h = ceil(sectionLength)};

	int hover = mouseX >= background.x && mouseY >= background.y &&
	            mouseX < background.x + background.w && mouseY < background.y + background.h;
	...
	SDL_SetRenderDrawColor(renderer, 139, 139, 139, 255);
	SDL_RenderDrawLine(renderer, foreground.x + foreground.w - 1, foreground.y,
	                   foreground.x + foreground.w - 1, foreground.y + foreground.h - 1);
}
...
int drawSlot(SDL_Renderer *renderer, InvSlot *slot, int x, int y, int mouseX, int mouseY) {
	int hover, i, xx, yy, color;

	char count[4];

	hover = mouseX >= x && mouseY >= y && mouseX < x + 16 && mouseY < y + 16;
 ...
 * of its buttons.
 */
int drawWorldListItem(SDL_Renderer *renderer, data_WorldListItem *item, int x, int y, int mouseX,
                      int mouseY) {
	int hover;

	hover = mouseX >= x && mouseY >= y && mouseX < x + 128 && mouseY < y + 16;
	...
	for (int yy = 0; yy < 16; yy++) {
		for (int xx = 0; xx < 16; xx++) {
			SDL_SetRenderDrawColor(renderer, (*pixel >> 16 & 0xFF), (*pixel >> 8 & 0xFF),
			                       (*pixel & 0xFF), 255);
			SDL_RenderDrawPoint(renderer, x + xx, y + yy);
			pixel++;
		}
	...
	if (button(renderer, "x", x + 128 - 16, y, 16, mouseX, mouseY)) {
		hover = 2;
	}
 ...
 */
void loadScreen(SDL_Renderer *renderer, const char *str, float prog, float max) {
	dirtBg(renderer);

	shadowCenterStr(renderer, str, BUFFER_HALF_W, BUFFER_HALF_H - 8);

	SDL_SetRenderDrawColor(renderer, 77, 77, 77, 255);
	SDL_RenderDrawLine(renderer, BUFFER_HALF_W - 32, BUFFER_HALF_H + 6, BUFFER_HALF_W + 32,
	                   BUFFER_HALF_H + 6);

	SDL_SetRenderDrawColor(renderer, 132, 255, 132, 255);
	SDL_RenderDrawLine(renderer, BUFFER_HALF_W - 32, BUFFER_HALF_H + 6,
	                   BUFFER_HALF_W - 32 + (prog / max) * 64, BUFFER_HALF_H + 6);
}
...
void chatAdd(const char *str) {
	...
	memcpy(chatHistory[(chatHistoryIndex)++], str, sizeof(char) * 64);
 ...
 */
void white(SDL_Renderer *renderer) { SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); }

/* tblack
 * Sets the render color to transparent black
 */
void tblack(SDL_Renderer *renderer) { SDL_SetRenderDrawColor(renderer, 0, 0, 0, 128); }
