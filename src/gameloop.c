...
/* File-scope shadows updated each frame so gl_renderer.c can read them */
static int _gl_blockSelected = 0;
static int _gl_blockSelectX  = 0;
static int _gl_blockSelectY  = 0;
static int _gl_blockSelectZ  = 0;
...
static uint32_t fps_lastmil = 0, fps_count = 0, fps_now = 0;
...
static void gameLoop_gameplay(SDL_Renderer *renderer, Inputs *inputs) {
	static double f21, f22, f23, f24, f25, f26, f27, f28, f29, f30, f31, f32, f33, f34, f35, f36;

	static int blockSelected = 0, selectedPass;
	...
	;
	int headInWater =
	    World_getBlock(&world, player->pos.x, player->pos.y, player->pos.z) == BLOCK_WATER;

	int feetInWater =
	    World_getBlock(&world, player->pos.x, player->pos.y + 1, player->pos.z) == BLOCK_WATER;
	...
	if (headInWater) {
		SDL_SetRenderDrawColor(renderer, 48 * timeCoef, 96 * timeCoef, 200 * timeCoef, 255);
	} else {
		...
		if (blockSelected) {
			...
			if (inputs->mouse.right > 0) {
				if (
				    ...
				    activeSlot->amount > 0) {
					int blockSet = World_setBlock(&world, blockSelectOffset.x, blockSelectOffset.y,
					                              blockSelectOffset.z, activeSlot->blockid, 1);
...
// Toggle advanced debug menu
		...
		if (inputs->keyboard.f) {
			inputs->keyboard.f = 0;
			InvSlot_swap(&player->inventory.hotbar[player->inventory.hotbarSelect],
		...
		for (int pixelY = 0; pixelY < BUFFER_H; pixelY++) {
			...
			for (int blockFace = 0; blockFace < 3; blockFace++) {
				...
				while (f33 < rayDistanceLimit) {
					...
					static IntCoords lookup_ago = {100000000, 100000000, 100000000}, lookup_now;

					lookup_now.x = blockRayPosition.x >> 6;
					lookup_now.y = blockRayPosition.y >> 6;
					lookup_now.z = blockRayPosition.z >> 6;

					if (lookup_now.x != lookup_ago.x || lookup_now.y != lookup_ago.y ||
						...
						int lookup_first = 0, lookup_last = CHUNKARR_SIZE - 1,
				...
				foundChunk:
					...
					if (intersectedBlock != BLOCK_AIR &&
						...
						if ((!blockSelected || blockRayPosition.x != blockSelect.x ||
						     blockRayPosition.y != blockSelect.y ||
						     blockRayPosition.z != blockSelect.z) ||
						    (textureX > 0 && textureY % 16 > 0 && textureX < 15 &&
						     textureY % 16 < 15) ||
						    !guiOn || gamePopup) {
							if (intersectedBlock >= NUMBER_OF_BLOCKS) {
								pixelColor = 0xFF0000;
							} else {
								pixelColor = textures[textureX + (textureY * 16) +
								                      intersectedBlock * 256 * 3];
							}
						}
						...
						if (f33 < f26 &&
						    ((!options.trapMouse && pixelX == inputs->mouse.x / BUFFER_SCALE &&
						      pixelY == inputs->mouse.y / BUFFER_SCALE) ||
						     (options.trapMouse && pixelX == BUFFER_HALF_W &&
	...
	/* Update file-scope shadows for gl_renderer */
	_gl_blockSelected = blockSelected;
	_gl_blockSelectX  = blockSelect.x;
	_gl_blockSelectY  = blockSelect.y;
	_gl_blockSelectZ  = blockSelect.z;
...
void gameLoop_drawPopup(SDL_Renderer *renderer, Inputs *inputs) {
	...
	case POPUP_HUD:
		...
		if (guiOn)
			popup_hud(renderer, inputs, &world, &debugOn, &fps_now, player);
		break;
...
static void gameLoop_processMovement(Inputs *inputs, int inWater) {
	...
	if (gamePopup == 0) {
		...
		if (options.trapMouse) {
			...
			double cameraMoveX = (inputs->mouse.x - BUFFER_W * 2) / (double)BUFFER_W * 2.0;
			double cameraMoveY = (inputs->mouse.y - BUFFER_H * 2) / (double)BUFFER_H * 2.0;

			double cameraMoveDistance =
			    sqrt(cameraMoveX * cameraMoveX + cameraMoveY * cameraMoveY) - 1.2;
	...
	if (doPhysics) {
		...
		playerMovement.x +=
		    player->vectorH.x * player->FBVelocity + player->vectorH.y * player->LRVelocity;
		playerMovement.z +=
		    player->vectorH.y * player->FBVelocity - player->vectorH.x * player->LRVelocity;
	...
	for (int axis = 0; axis < 3; axis++) {
		...
		for (int step = 0; step < 12; step++) {
			int blockX = floor(playerPosTry.x + ((step >> 0) & 1) * 0.6 - 0.3);
			int blockY = floor(playerPosTry.y + ((step >> 2) - 1) * 0.8 + 0.65);
			int blockZ = floor(playerPosTry.z + ((step >> 1) & 1) * 0.6 - 0.3);
...
int gameLoop_screenshot(SDL_Renderer *renderer, const char *path) {
	SDL_Surface *grab = SDL_CreateRGBSurfaceWithFormat(
	    0, BUFFER_W * BUFFER_SCALE, BUFFER_H * BUFFER_SCALE, 32, SDL_PIXELFORMAT_ARGB8888);
...
void gameLoop_error(char *message) { errorMessage = message; }
/* -------------------------------------------------------------------------
 * GL renderer accessors - expose static gameplay state to gl_renderer.c
 * ---------------------------------------------------------------------- */
int gl_getBlockSelected(void) { return _gl_blockSelected; }
void gl_getBlockSelectCoords(int *x, int *y, int *z) {
	*x = _gl_blockSelectX;
	*y = _gl_blockSelectY;
	*z = _gl_blockSelectZ;
}
int gl_getGuiOn(void) { return guiOn; }
int gl_getDebugOn(void) { return debugOn; }